...
export default function Envios() {
  ...
  return (
    <div className="container animate-fade-in" style={{ marginTop: '50px', textAlign: 'center' }}>
      ...
      <p style={{ color: 'var(--muted-foreground)', marginBottom: '40px' }}>Por favor escoge cómo deseas recibir tu pedido en tu domicilio.</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
        ...
        <div className="card" style={{ padding: '30px', cursor: 'pointer', border: '2px solid transparent' }} onClick={() => handleEnvio('Estandar')}>
          ...
          <h2 style={{ color: 'var(--success)', margin: '15px 0' }}>Gratis</h2>
          <p style={{ color: 'var(--muted-foreground)' }}>Llega entre 3 a 5 días hábiles.</p>
        </div>

        <div className="card" style={{ padding: '30px', cursor: 'pointer', border: '2px solid transparent' }} onClick={() => handleEnvio('Express')}>
          <h3>Envío Express</h3>
          <h2 style={{ color: 'var(--success)', margin: '15px 0' }}>$1,500</h2>
          <p style={{ color: 'var(--muted-foreground)' }}>Recíbelo en menos de 24 horas.</p>
        </div>

        <div className="card" style={{ padding: '30px', cursor: 'pointer', border: '2px solid transparent' }} onClick={() => handleEnvio('Tienda')}>
          <h3>Retiro en Tienda</h3>
          <h2 style={{ color: 'var(--success)', margin: '15px 0' }}>Gratis</h2>
          <p style={{ color: 'var(--muted-foreground)' }}>Retíralo hoy mismo en nuestra sucursal técnica.</p>
        </div>

      </div>
      
      <button style={{ background: 'none', border: 'none', color: 'var(--destructive)', marginTop: '40px', fontWeight: 'bold', cursor: 'pointer' }} onClick={() => navigate('/cancel-purchase')}>
        Cancelar Todo
      </button>
    </div>
  );
